...
>
> ⚠️ **Ollama 컨테이너 제거됨** — LLM은 외부 API(Gemini/OpenAI/Claude) 사용.
> GPU/RAM 요구사항이 대폭 낮아지고, 이미지 빌드 크기도 ~3.4GB 줄어듭니다.
...
LLM은 외부 API(Gemini / OpenAI / Claude)를 사용하므로 별도 컨테이너 불필요.
API 키는 Docker 시크릿 또는 `.env` 파일로 주입합니다.

### 1.2 최소 하드웨어 요구사항

| 구성 | 최소 | 권장 | Ollama 대비 변화 |
|------|------|------|----------------|
| CPU | 2코어 | 4코어+ | ↓ 완화 |
| RAM | 4GB | 8GB | ↓ 대폭 완화 (16GB→4GB) |
| 스토리지 | 10GB | 20GB | ↓ 대폭 완화 (50GB→10GB) |
| GPU | 불필요 | 불필요 | ↓ 완전 제거 |
| OS | Ubuntu 22.04+ | Ubuntu 24.04 LTS | 동일 |
...
│   └── proxy/  # ollama/ 디렉토리 제거됨 — 외부 LLM API 사용
...
# ⚠️ ollama 서비스 제거됨 — LLM은 외부 API(Gemini/OpenAI/Claude) 사용
# GPU/RAM 요구사항 불필요, 이미지 빌드 크기 ~3.4GB 절감
...
services:
  api:
    ...
    ports:
      - "8000:8000"
    environment:
      # LLM API 설정 (사용할 프로바이더 키 하나 이상 필수)
