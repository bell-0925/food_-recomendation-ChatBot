...
**단일 Tool 호출 방식 (Native Function Calling):**

Gemini / OpenAI / Claude 모두 Native Function Calling을 완벽 지원합니다.
`llm_client.py`의 멀티 프로바이더 추상화 레이어가 각 SDK 포맷 차이를 자동으로 처리합니다.

> ⚠️ Ollama 기반 "프롬프트 기반 Fallback" 방식은 제거되었습니다.
> 외부 API는 모두 Native Function Calling을 지원하므로 불필요합니다.
...
├── tool_definitions.py         # 신규: OpenAI 호환 Tool 스키마 (Gemini/OpenAI/Claude 공통)
...
└── prompts/
    ...
    # tool_fallback.py, tool_prompt.md 제거 (외부 API는 Native FC 지원)
